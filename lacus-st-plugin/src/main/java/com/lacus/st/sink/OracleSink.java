...
/**
 * Oracle Sink 组件
 * 属性对齐 SeaTunnel 2.3.12 Oracle(Jdbc) Sink Options。
 * 文档：https://seatunnel.apache.org/zh-CN/docs/2.3.12/connector-v2/sink/Oracle
 *
 * @author lacus
 */
@Data
...
@StComponent(
        ...
        description = "通过 JDBC 写入 Oracle，支持批处理/流模式、精确一次(XA)",
        version = "2.0.0",
        author = "lacus",
        connectorKey = "Jdbc"
)
@StTag({
        ...
        @TagDefinition(name = "性能配置", displayName = "性能配置", order = 3, description = "batch_size/batch_interval_ms/connection_check_timeout_sec/max_retries"),
        @TagDefinition(name = "表结构配置", displayName = "表结构配置", order = 4, description = "schema_save_mode/data_save_mode/custom_sql"),
...
public class OracleSink extends AbstractStSink {
    ...
    @StField(tag = "性能配置", order = 2, required = false, enName = "batch_interval_ms", cnName = "批间隔(毫秒)",
            defaultValue = "1000", description = "缓冲达到 batch_size 或达到该时间时刷新",
            placeHolder = "1000", formType = StField.FormType.POSITIVE_NUMBER)
    private Integer batchIntervalMs;

    @StField(tag = "性能配置", order = 3, required = false, enName = "connection_check_timeout_sec", cnName = "连接检查超时(秒)",
            defaultValue = "30", placeHolder = "30", formType = StField.FormType.POSITIVE_NUMBER)
    private Integer connectionCheckTimeoutSec;

    @StField(tag = "性能配置", order = 4, required = false, enName = "max_retries", cnName = "最大重试次数",
    ...
    @StField(tag = "表结构配置", order = 3, required = false, enName = "custom_sql", cnName = "自定义 SQL",
            description = "data_save_mode=CUSTOM_PROCESSING 时填写",
    ...
    @StField(tag = "事务配置", order = 2, required = false, enName = "xa_data_source_class_name", cnName = "XA 数据源类名",
            ...
            placeHolder = "oracle.jdbc.xa.client.OracleXADataSource", formType = StField.FormType.TEXT)
    ...
    @StField(tag = "其他", order = 3, required = false, enName = "properties", cnName = "附加连接参数",
            placeHolder = "key=value", formType = StField.FormType.TEXT)
    ...
    public JSONObject buildTaskConfig(JSONObject connectionConfig, Long datasourceId) {
        ...
        putIfNotEmpty(config, "batch_interval_ms", connectionConfig.getInteger("batch_interval_ms"));
