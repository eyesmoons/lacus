...
/**
 * PostgreSQL Sink 组件
 * 属性对齐 SeaTunnel 2.3.12 PostgreSql(Jdbc) Sink Options。
 * 文档：https://seatunnel.apache.org/zh-CN/docs/2.3.12/connector-v2/sink/PostgreSql
 *
 * @author lacus
 */
@Data
...
@StComponent(
        ...
        description = "通过 JDBC 写入 PostgreSQL，支持批处理/流模式、精确一次(XA)",
        version = "2.0.0",
        author = "lacus",
        connectorKey = "Jdbc"
)
...
public class PostgreSqlSink extends AbstractStSink {
    ...
    @StField(tag = "数据配置", order = 1, required = false, enName = "query", cnName = "SQL 写入语句",
            description = "使用 json/jsonb 插入时可在 url 加 stringtype=unspecified",
    ...
    @StField(tag = "事务配置", order = 2, required = false, enName = "xa_data_source_class_name", cnName = "XA 数据源类名",
            ...
            placeHolder = "org.postgresql.xa.PGXADataSource", formType = StField.FormType.TEXT)
    ...
    @StField(tag = "其他", order = 3, required = false, enName = "properties", cnName = "附加连接参数",
            placeHolder = "stringtype=unspecified", formType = StField.FormType.TEXT)
