...
import com.lacus.utils.BeanUtils;
import com.lacus.utils.ExceptionUtil;
...
public abstract class AbstractQuartzJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        // 判断是字符串还是对象，兼容新旧两种方式
        if (taskProperties instanceof String) {
            // 新方式：从 JSON 字符串反序列化
            sysJob = JSON.parseObject((String) taskProperties, SysJob.class);
        } else {
            // 旧方式：直接复制属性（兼容性保留）
    ...
    protected void after(JobExecutionContext context, SysJob sysJob, Exception e) {
        ...
        if (e != null) {
            sysJobLog.setStatus("1");  // 1表示失败
            String errorMsg = StringUtils.substring(ExceptionUtil.getExceptionMessage(e), 0, 2000);
            sysJobLog.setExceptionInfo(errorMsg);
        } else {
            sysJobLog.setStatus("0");  // 0表示成功
        }
